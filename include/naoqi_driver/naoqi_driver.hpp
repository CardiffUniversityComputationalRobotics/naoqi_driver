...
namespace naoqi
{
  ...
  public:
    ...
    // define bool to publish odom tf
    void setOdomTfPub(const bool pubOdom);
  ...
  private:
    ...
    {
      ScheduledConverter(const ros::Time &schedule, size_t conv_index) : schedule_(schedule), conv_index_(conv_index)
      {
      }
