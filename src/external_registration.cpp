...
int main(int argc, char **argv)
{
  ...
  desc.add_options()("help,h", "print help message")("nao_ip", po::value<std::string>(), "the ip of the robot")("nao_port", po::value<int>(), "the ip of the robot")("user,u", po::value<std::string>(), "the user profile on the robot, nao by default")("password,p", po::value<std::string>(), "the password of the robot")("roscore_ip,r", po::value<std::string>(), "set the ip of the roscore to use")("network_interface,i", po::value<std::string>()->default_value("eth0"), "set the network interface over which to connect")("namespace,n", po::value<std::string>()->default_value("naoqi_driver_node"), "set an explicit namespace in case ROS namespace variables cannot be used")("publish_odom_tf,o", po::value<std::string>()->default_value("true"), "set if odom transform is desired");
  ...
  if (vm.count("help"))
  {
    std::cout << "This is the help message for the ROS-Driver C++ bridge" << std::endl
              << desc << std::endl;
    exit(0);
  }
  ...
  std::string publish_odom_tf = vm["publish_odom_tf"].as<std::string>();
  if (publish_odom_tf == "true")
  {
    bs->setOdomTfPub(true);
  }
  else
  {
    bs->setOdomTfPub(false);
  }
